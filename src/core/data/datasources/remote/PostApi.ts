...
import {
  ...
  GetMoodRecommendedPostsResponse,
} from "../../dtos/PostDto";
...
const PostApi = {
  ...
  getMoodRecommendedPosts: async (
    params: PageRequest
  ): Promise<GetMoodRecommendedPostsResponse> => {
    const { data } = await ApiService.get<GetMoodRecommendedPostsResponse>(
      API_ENDPOINTS.GET_MOOD_RECOMMENDED_POSTS,
      { params: { PageIndex: params.pageIndex, PageSize: params.pageSize } }
    );
    return data;
  },
